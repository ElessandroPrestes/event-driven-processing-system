...
- Health check com readiness real de PostgreSQL, Redis, RabbitMQ e heartbeat dos workers obrigatorios, evitando falso positivo de aplicacao saudavel enquanto o pipeline assincrono esta degradado.
...
- API operacional autenticada para inspecao e replay da dead-letter queue, cobrindo tanto eventos persistidos quanto mensagens órfãs ou inválidas, inclusive com replay direcionado por `message_id`.
...
- Sanitizar o payload de readiness para expor somente o necessario ao diagnostico operacional sem vazar segredos, hostnames internos ou stack traces.
